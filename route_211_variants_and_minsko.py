...
def get_routes_for_target_stops(route_names=None, stop_name=None):
    extract_gtfs()
    ...
    # Case 1: filter by route names like ["211", "211A"]
    if route_names:
        filtered_routes = routes[routes['route_short_name'].isin(route_names)]
    ...
    # Case 2: filter by stop name like "Minsko plentas"
    elif stop_name:
        filtered_stops = stops[stops['stop_name'].str.lower().str.contains(stop_name.lower())]
        stop_ids = filtered_stops['stop_id'].unique()
    else:
        raise Exception("Specify route_names or stop_name.")
...
# Export DOCX for route 211, 211A, 211B
route_211_variants = ["211", "211A", "211B"]
route_data = get_routes_for_target_stops(route_names=route_211_variants)
for stop, sched in route_data.items():
    export_schedule_to_docx(stop, sched)

# Also export Minsko plentas
minsk_data = get_routes_for_target_stops(stop_name="Minsko plentas")
