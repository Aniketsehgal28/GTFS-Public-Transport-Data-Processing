...
def get_schedule_for_route(route_prefix):
    ...
    enriched = stop_times.merge(trips[['trip_id', 'route_id', 'service_id', 'direction_stop_name']], on='trip_id')
    ...
    enriched['service_flags'] = enriched.apply(get_service_flags, axis=1)

    stop_data = defaultdict(lambda: defaultdict(list))

    for trip_id, group in enriched.groupby('trip_id'):
        trip_stops = group.sort_values('stop_sequence')
        last_stop = trip_stops['direction_stop_name'].iloc[0].strip()
        ...
        for _, row in trip_stops.iterrows():
            ...
            try:
                ...
                for flag in row['service_flags']:
                    stop_data[stop_name][last_stop].append((route, time, flag))
            except:
                continue

    # Organize by route and flags
