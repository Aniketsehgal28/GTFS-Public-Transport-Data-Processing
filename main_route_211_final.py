# We'll create a new version of `main.py` that includes route 211 logic and DOCX export in the same style.
import os
import zipfile
import pandas as pd
...
def get_route_211_data():
    ...
    # Adjust this if route 211 is labeled differently
    route_211 = routes[routes['route_short_name'].astype(str).str.contains("211", case=False)]
    if route_211.empty:
        raise Exception("Route 211 not found.")

    route_ids = route_211['route_id'].unique()
    trips_211 = trips[trips['route_id'].isin(route_ids)]
    trip_ids = trips_211['trip_id'].unique()
    stop_times = stop_times[stop_times['trip_id'].isin(trip_ids)]

    merged = stop_times.merge(trips_211[['trip_id', 'route_id', 'service_id']], on='trip_id')
...
def create_gui(stop_data):
    ...
    root = tk.Tk()
    root.title("Route 211 Bus Timetable")
    root.geometry("800x550")
    root.configure(bg="#f8f9fa")

    ttk.Label(root, text="🚌 Route 211 Bus Timetable", font=("Segoe UI", 16, "bold")).pack(pady=10)
...
if __name__ == "__main__":
    stop_data = get_route_211_data()
    create_gui(stop_data)
# This code is a complete implementation of the Route 211 bus timetable viewer and exporter.
# It extracts GTFS data, processes it to find stops served by route 211, and provides a GUI for users to view and export the timetable.